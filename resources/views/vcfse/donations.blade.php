...
<div class="card">
  <div class="card-hd">
    ...
    <a href="{{ route('vcfse.donate') }}" class="btn btn-primary btn-sm"><i class="fas fa-plus"></i> New Donation</a>
  </div>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            ...
            @elseif($d->status === 'failed')<span class="badge badge-red">Failed</span>
            @else<span class="badge badge-gray">{{ ucfirst($d->status) }}</span>@endif
          </td>
        </tr>
        @empty
        <tr><td colspan="5"><div class="empty-state"><div class="empty-icon"><i class="fas fa-receipt"></i></div><h3>No donations yet</h3><p><a href="{{ route('vcfse.donate') }}" style="color:#16a34a;font-weight:600">Make your first donation</a></p></div></td></tr>
        @endforelse
      </tbody>
    </table>
  </div>
