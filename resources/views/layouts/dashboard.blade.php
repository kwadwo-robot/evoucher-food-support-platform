...
<aside class="sidebar" :class="sidebarOpen ? 'open' : ''">
  ...
  <a href="{{ route('admin.service-fees.index') }}" class="nav-item {{ request()->routeIs('admin.service-fees.*') ? 'active' : '' }}">
    <span class="ni"><i class="fas fa-percent"></i></span> {{ __('app.service_fees') }}
  </a>
      ...
      <div x-show="open" x-cloak @click.away="open=false"
        ...
        <div x-data="{ langOpen: false }" class="relative">
          ...
          <div x-show="langOpen" x-cloak @click.away="langOpen=false"
            ...
            <a href="#" class="lang-option" onclick="switchLanguage('en', event)" class="flex items-center gap-2 px-4 py-2 text-sm {{ app()->getLocale()==='en' ? 'text-green-600 font-semibold bg-green-50' : 'text-slate-700 hover:bg-slate-50' }}" data-lang="en">🇬🇧 English</a>
            <a href="#" class="lang-option" onclick="switchLanguage('ar', event)" class="flex items-center gap-2 px-4 py-2 text-sm {{ app()->getLocale()==='ar' ? 'text-green-600 font-semibold bg-green-50' : 'text-slate-700 hover:bg-slate-50' }}" data-lang="ar">🇸🇦 العربية</a>
            <a href="#" class="lang-option" onclick="switchLanguage('ro', event)" class="flex items-center gap-2 px-4 py-2 text-sm {{ app()->getLocale()==='ro' ? 'text-green-600 font-semibold bg-green-50' : 'text-slate-700 hover:bg-slate-50' }}" data-lang="ro">🇷🇴 Română</a>
            <a href="#" class="lang-option" onclick="switchLanguage('pl', event)" class="flex items-center gap-2 px-4 py-2 text-sm {{ app()->getLocale()==='pl' ? 'text-green-600 font-semibold bg-green-50' : 'text-slate-700 hover:bg-slate-50' }}" data-lang="pl">🇵🇱 Polski</a>
          </div>
        </div>
        <div class="border-t border-slate-100 my-1"></div>
        <form method="POST" action="{{ route('logout') }}">
