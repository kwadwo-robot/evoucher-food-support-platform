...
class DashboardController extends Controller
{
    ...
    private function resolveVoucher(string $code): array
    {
        ...
        // Get all redeemed item IDs for this voucher to exclude them
        $redeemedItemIds = $redemptionHistory->pluck('food_listing_id')->toArray();
        
        // Get available food listings at this shop, excluding already-redeemed items
        $foodListings = FoodListing::where('shop_user_id', Auth::id())
            ->where('status', 'available')
            ->whereNotIn('id', $redeemedItemIds)
            ->orderBy('expiry_date')
            ->get();
        
        // If no items are available (all have been redeemed), show empty collection
        // This prevents the voucher from being used on already-collected items
