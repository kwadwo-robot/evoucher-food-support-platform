...
class VoucherController extends Controller
{
    ...
    public function create()
    {
        ...
        $walletBalance = 0;
        if ($user->role === 'vcfse' && $user->vcfse_organisation_id) {
            $organisation = \App\Models\VcfseOrganisation::find($user->vcfse_organisation_id);
            $walletBalance = $organisation ? $organisation->wallet_balance : 0;
        } elseif ($user->role === 'school' && $user->school_id) {
            $school = \App\Models\School::find($user->school_id);
            $walletBalance = $school ? $school->wallet_balance : 0;
        }
    ...
    private function storeManualVoucher(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'new_recipient_email' => 'required|email|unique:users,email',
            'value' => 'required|numeric|min:0.01',
            'quantity' => 'nullable|integer|min:1|max:100',
            'expiry_days' => 'required|integer|min:1|max:365',
            'notes' => 'nullable|string|max:500',
        ...
        try {
            ...
            $recipient = User::create([
                'name' => $validated['new_recipient_name'],
                'email' => $validated['new_recipient_email'],
    ...
    {
        if ($user->role === 'vcfse') {
            // VCFSE can issue to recipients they support
            return User::where('role', 'recipient')
                ->where('is_active', true)
                ->orderBy('name')
                ->get();
        } elseif ($user->role === 'school') {
            // Schools can issue to recipients in their area
            return User::where('role', 'recipient')
                ->where('is_active', true)
                ->orderBy('name')
                ->get();
    ...
    {
        if ($recipient->role !== 'recipient' || !$recipient->is_active) {
            return false;
        }
    ...
    public function show($id)
    {
        ...
        // Check authorization
        if (Auth::id() !== $voucher->issued_by && Auth::id() !== $voucher->recipient_user_id) {
